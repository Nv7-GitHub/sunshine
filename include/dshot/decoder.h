  ...
  bool decodeTelemetry(const uint64_t& raw_telemetry,
    ...
    for (uint64_t mask = 0x4000000000000000; mask; mask = (mask >> 1)) {
      if (((raw_telemetry & mask) != 0) !=
          current) {  // if the doesn't match the current string of bits, end
                      // the current string and flip current
    ...
    if (bits_found < 21) {
      Serial.print(" Too few bits found: ");
      Serial.println(bits_found);
      telemetry.errors++;
      return false;
    }
