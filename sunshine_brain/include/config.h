...
// AM32 2.x auto-detects BDSHOT from the inverted CRC — no configurator setting
// needed.
#define DSHOT_BIDIRECTIONAL 1

// ── Battery ADC
// ─────────────────────────────────────────────────────────────── ADC1_CH7 =
// GPIO7, connected to battery voltage divider (R_high=2k, R_low=1k)
...
// ── Motor direction
// ─────────────────────────────────────────────────────────── Set to true to
// invert a motor's DShot output (mirrors forward↔reverse around DSHOT_NEUTRAL).
// Use this when wiring or AM32 direction can't be changed. In MELTY mode with
// no translation input, both motors spinning "forward" should produce CCW body
...
// pre-pairing bootstrap: broadcast (FF:..) until the first control packet
// arrives, after which telemetry switches to UNICAST to the learned MAC (→ MAC
// ACK + hardware retransmit = no lost frames). Leave as broadcast unless you
// want to hard-pin a specific receiver.
static const uint8_t RECEIVER_MAC[6] = {0x44, 0x1B, 0xF6, 0xD6, 0x46, 0x50};
