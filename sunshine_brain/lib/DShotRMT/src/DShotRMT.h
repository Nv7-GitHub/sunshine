...
public:
    ...
    // Dumps the last received RX frame symbols to Serial (debug only).
    void dumpLastRxFrame() const;
...
private:
    ...
    // Per-instance RX buffer — must NOT be static/shared; two motors receiving
    // simultaneously would corrupt each other's data with a shared buffer.
    rmt_symbol_word_t _rx_symbols[48];

    // Snapshot of the last received frame for debug dumping (copied in ISR).
    rmt_symbol_word_t _last_rx_snapshot[48];
    ...
    uint16_t IRAM_ATTR _decodeDShotFrame(const rmt_symbol_word_t *symbols, size_t num_symbols) const;            // Decodes a received RMT symbol array into an eRPM value
